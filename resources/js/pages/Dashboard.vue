...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div
            class="grid grid-cols-1 gap-6 p-5 p-6 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4"
        >
            <div
                v-for="metric in metrics"
                :key="metric.label"
                class="flex items-center justify-between rounded-lg bg-white p-6 shadow"
            >
                <div>
                ...
                <div
                    :class="`text-${metric.color}-500 bg-${metric.color}-100 rounded-full p-2`"
                >
